...
$(function(){
					...
					$('#catalogContent').find('li').bind('dblclick',function(){
						...
						if (curFileType != fileType.directory){
							...
							if (isLeaf == "true"){
								...
								$('#catalogContent').html("");
								var userId=document.getElementById("userId").value;
								var url="/hdfs/file/searchfile?fileName="+curFileName+"&userId="+userId;
							    window.location.href=url;
		...
		function eachDirectory(dir,parentId){
			//alert("eachDirectory");
			{
				//alert("dir.parentId="+dir.parentId);
					filterDir.push(dir);
					return;
				}
					if (dir.isLeaf == "false"){
						...
						for (var j = 0,lens = children.length; j < lens; j++){
							...
							//alert("eachDirectory="+eachDirectory(children[j],parentId));
						}
				}
			}	
		...
		function findFile(parentId){
			...
			//alert(curdir);
			/*var fileName = $('.curdir').attr('fileName');
			alert(fileName);*/
			//alert('findFile');
			filterDir = [];
			 for (var i = 0,len = curdir.length; i < len; i++){
				 //alert("length="+curdir.length);
